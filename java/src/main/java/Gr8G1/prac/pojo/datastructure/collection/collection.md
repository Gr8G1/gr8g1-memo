...
- ArrayList
  ...
  - 인덱스 기반 조회·수정과 맨 끝에 추가(amortized O(1))는 매우 빠르다. (단, 중간 삽입/삭제는 아래처럼 느리다.) 
...
- LinkedList (Queue 인터페이스를 구현한 클래스)
  ...
  - 삽입·삭제 위치의 노드 참조를 이미 알고 있으면 앞뒤 링크만 바꾸면 되므로 ArrayList처럼 뒤 데이터를 밀어낼 필요가 없다. 다만 임의 인덱스에 삽입하려면 그 위치까지 O(n) 탐색이 먼저 필요하므로 "바로" 삽입되는 것은 아니다. 
...
- TreeSet
  ...
  - 값들이 정렬 기준(natural ordering 또는 Comparator)에 따라 자동 정렬되어 저장된다. 즉 '삽입한 순서'는 보존하지 않지만 '정렬 순서'는 보장한다. 
