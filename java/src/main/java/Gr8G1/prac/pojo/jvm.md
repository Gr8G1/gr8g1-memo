...
- 처리 과정
  1. Java compiler(javac)가 Java 원시코드(*.java)를 바이트코드 클래스파일(*.class)로 컴파일한다.
  2. JVM의 **실행 엔진**이 이 바이트코드를 런타임에 실행한다. 처음에는 인터프리터로 한 줄씩 해석하다가, 자주 실행되는 핫스팟(hotspot)을 **JIT 컴파일러**가 기계어로 컴파일해 이후로는 빠르게 직접 실행한다. (즉 .class→기계어 변환은 별도의 '컴파일 단계'가 아니라 실행 엔진의 런타임 동작이다.)
...
1. 클래스 로더(Class Loader)
   ...
   - 플랫폼 클래스 로더(Platform Class Loader)
     - JDK의 플랫폼(확장) 모듈·라이브러리를 로드한다. (Java 8까지는 '확장 클래스 로더(Extension Class Loader)'였으나, Java 9 모듈 시스템 도입과 함께 Platform Class Loader로 대체됨)
...
- 메소드(method) 영역 : 클래스에 대한 정보와 함께 클래스 변수(static variable)가 저장되는 영역
  ...
  - Java 8부터 이 영역은 힙이 아닌 **네이티브 메모리의 Metaspace**로 구현된다(구 PermGen 대체).
...
- Xmx: 최대 Heap size
  ...
  - 64bit 운영체제인 경우 최대 Heap Size : 이론상 매우 큼 **(단 Compressed Oops 효율을 위해 32GB 이하 권장)**
