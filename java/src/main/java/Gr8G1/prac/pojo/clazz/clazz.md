...
- 제네릭(generic)
  - 클래스나 메소드에서 사용할 데이터 타입을 구체적으로 명시하지 않고 **타입 파라미터로 일반화**하는 기능이다. 실제 타입은 **컴파일 타임에 결정·검사**되며, 런타임에는 타입 정보가 지워진다(타입 소거, type erasure).
...
- 메소드(method)
  - 선언 형식
    [접근 제어자] [static] [final] 반환타입 메소드명(매개변수목록) { ... }
  - **시그니처(signature)**: 메소드명 + 매개변수 목록(타입·개수·순서). 반환타입·접근제어자는 시그니처에 포함되지 않으며, 오버로딩을 식별하는 기준이 된다.
  - 메소드 오버로딩
    같은 이름의 메소드를 매개변수 목록을 달리하여 중복 정의하는 것을 의미한다.
  - 메소드 오버로딩 요구조건
    1. 메소드의 이름이 같아야 한다.
