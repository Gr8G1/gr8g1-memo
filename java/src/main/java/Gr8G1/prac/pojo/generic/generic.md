...
- 와일드 카드 (범위 제한)
  ...
  - <?>: 모든 타입 허용 (비한정 와일드카드, 대략 `<? extends Object>`에 해당)
...
- 제네릭 타입 컨벤션
  ...
  - T: Type
  - V: Value
  - S, U: 2nd, 3rd 추가 타입 파라미터 (관례상 T 다음 S, U, V 순서로 사용)
...
3. 제네릭 타입으로 직접 인스턴스/배열 생성 불가
   타입 소거 때문에 런타임에 실제 타입을 알 수 없어 `new T()`, `new T[10]` 은 불가능하다. (필요하면 `Class<T>`나 `Supplier<T>`를 받아 생성하거나 `(T[]) new Object[10]` 으로 우회한다.)
