...
2. @Async 
   ...
   - 반환 타입은 `void` 또는 `Future`/`CompletableFuture<T>`가 권장된다(`CompletableFuture`가 조합·예외 처리에 유리).
   - **주의(self-invocation)**: `@Async`는 프록시로 동작하므로, **같은 클래스 내부에서 자기 메서드를 직접 호출하면 비동기로 동작하지 않는다.** 다른 빈을 통해 호출해야 한다.
