...
#### 종류
1. Authorization Code Grant(권한 부여 승인 코드 방식) — ✅ **현재 표준/권장**
   - 권한 부여 승인을 위해 자체 생성한 Authorization Code를 전달하는 방식으로 가장 보편화된 방식이며, Refresh Token을 사용할 수 있다.
   - 권한 부여 승인 요청시 응답 타입(response_type)을 code 로 지정하여 요청한다.
   - 공개 클라이언트(SPA·모바일)는 인가 코드 가로채기를 막는 **PKCE(Proof Key for Code Exchange)** 확장을 함께 쓰는 것이 현재 표준이다.
2. Implicit Grant(암묵적 승인 방식) — ⚠️ **현재는 사용 비권장(deprecated)**
   - 별도의 Authorization Code 없이 바로 Access Token을 발급하는 방식으로, 과거 자격증명을 안전하게 저장하기 힘든 Client(자바스크립트 등 스크립트 언어를 사용하는 브라우저)에 쓰였다.
   ...
   - **주의:** 토큰이 URL(프래그먼트)에 노출되는 보안 문제로 OAuth 2.0 Security BCP에서 비권장되고 **OAuth 2.1에서 제거**되었다. 현재 브라우저/SPA는 **Authorization Code + PKCE**를 사용한다.
...
4. Resource Owner Password Credentials(자원 소유자 자격 증명 승인 방식) — ⚠️ **현재는 사용 비권장(deprecated)**
   - 로그인 시 필요한 정보(username, password)로 Access Token을 발급받는 방식. 자신의 서비스에서 제공하는 애플리케이션의 경우에만 제한적으로 쓰였으며, Refresh Token의 사용도 가능하다.
   - Authorization Server, Resource Server, Client가 모두 같은 시스템에 속해 있을 때만 사용이 가능하다.
   - **주의:** 비밀번호를 클라이언트가 직접 다루는 구조라 OAuth 2.0 Security BCP에서 비권장되고 OAuth 2.1에서 제거되었다. 역시 **Authorization Code + PKCE**로 대체한다.
