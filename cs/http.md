...
### 특징
#### 비연결성(Connectionless)
클라이언트와 서버 간의 연결을 계속 유지하지 않고, 요청(request)과 응답(response)을 완료한 후에 연결을 끊는 것이 기본 개념이다. 
이는 매번 새로운 연결을 맺어야 한다는 단점이 있지만, 연결을 유지하지 않아 서버의 자원을 효율적으로 사용할 수 있다.

> 단, 이는 HTTP/1.0 기준이다. HTTP/1.1부터는 **keep-alive**로 한 연결을 재사용하고, HTTP/2는 하나의 연결에서 여러 요청을 동시에 처리(멀티플렉싱)하므로 매 요청마다 연결을 끊지 않는다. (아래 'HTTP 버전' 참고)

#### 무상태(Stateless)
클라이언트와 서버 간의 상태 정보를 유지하지 않는다. 각각의 요청(request)과 응답(response)은 서로 **독립적**이어서, 서버는 이전 요청의 맥락을 기억하지 않고 각 요청을 그 자체로 완전한 정보로 처리한다. 
이러한 무상태성은 서버 확장이 쉽고 부하를 줄이는 장점이 있지만, 로그인 상태처럼 맥락을 이어가야 할 때는 별도 수단(쿠키·세션·토큰)이 필요하다.
...
| 버전 | 핵심 | 연결 관리 |
| --- | --- | --- |
| HTTP/1.0 | 요청마다 새 연결 | 비연결(요청 후 연결 종료) |
| HTTP/1.1 | keep-alive, 파이프라이닝 | 연결 재사용. 단 응답이 순서대로 처리되어 **HOL 블로킹(Head-of-Line)** 존재 |
| HTTP/2 | **멀티플렉싱**(한 연결에 여러 요청 동시), 헤더 압축(HPACK), 서버 푸시 | 단일 연결로 병렬 처리(애플리케이션 계층 HOL 해소) |
| HTTP/3 | **QUIC(UDP 기반)** 위에서 동작, 0-RTT 핸드셰이크 | TCP가 아닌 UDP라 **TCP 레벨 HOL 블로킹까지 제거**, 연결 마이그레이션 지원 |

- HTTP/2는 TCP 위에서 동작하므로 TCP 단의 패킷 손실 시 HOL 블로킹이 남는다. HTTP/3는 전송 계층을 **QUIC(UDP 기반)** 로 바꿔 이를 해결했다.
- HTTP/2 이상은 사실상 HTTPS(TLS) 위에서 사용된다.
...
> 버전 주의: 이름은 'SSL'로 굳었지만 **SSL 전 버전과 TLS 1.0/1.1은 취약점으로 폐기(deprecated)** 되었고, 현재는 **TLS 1.2와 TLS 1.3**을 사용한다(1.3은 핸드셰이크가 더 빠르고 안전).
