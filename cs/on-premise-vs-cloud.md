...
### AWS
  - AWS는 2006년 S3(Simple Storage Service) IaaS(Infrastructure as a Service) 퍼블릭 클라우드 시장을 탄생시켰고
    ...
    AWS는 전 세계 여러 **리전(Region)** 과 각 리전 내 다수의 **가용영역(AZ, Availability Zone)** 을 운영하며 그 수는 계속 늘고 있습니다. (리전=지리적 지역, AZ=리전 내 독립 데이터센터 묶음)
    AWS는 아마존 EC2(Elastic Compute Cloud) 가상 머신, EC2(Elastic Compute Cloud) 컨테이너 서비스, 가상 프라이빗 서버, 람다(Lambda) 서버리스 컴퓨팅 플랫폼 등 '컴퓨팅',
    S3와 일래스틱 블록 스토리지, 경제적인 아카이브인 글래시어 등 '스토리지, 관계형 데이터베이스인 오로라(Aurora)와 MySQL용 아마존 RDS, PostgreSQL, 오라클, SQL 서버, NoSQL 데이터베이스인 다이나모 DB(DynamoDB) 등 데이터베이스까지 수많은 기능을 자랑합니다.
...
5. 멀티 테넌시(Multi tenancy)
   - 한 제공자의 인프라(자원)를 다수의 사용자(테넌트)가 논리적으로 분리된 채 공유하여 사용하는 특성이다. (여러 클라우드 제공자의 자원을 합치는 것이 아니라, 한 제공자의 자원을 여러 테넌트가 나눠 쓰는 것)
