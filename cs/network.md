...
- Ex: Host A(Client) <=> Host B(Server) — 연결 설정(3-way)
  1. A → B : **SYN** (연결 요청, 시퀀스 번호 전송)
  2. B → A : **SYN + ACK** (요청 수락 + 확인 응답)
  3. A → B : **ACK** (확인 응답) → 연결 수립
  - 연결 **해제**는 4-way handshaking으로 **FIN ↔ ACK**를 주고받는다. (FIN은 해제 신호이며 연결 설정 단계에는 등장하지 않는다.)
