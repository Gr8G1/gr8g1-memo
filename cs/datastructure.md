...
### 자료구조별 시간 복잡도(Big-O)
연산별 평균/최악 시간 복잡도. 자료구조 선택의 핵심 기준이다.

| 자료구조 | 접근(Access) | 탐색(Search) | 삽입(Insert) | 삭제(Delete) | 비고 |
| --- | --- | --- | --- | --- | --- |
| 배열(Array) | O(1) | O(n) | O(n) | O(n) | 인덱스 접근 O(1), 맨 끝 추가는 평균 O(1) |
| 연결 리스트(Linked List) | O(n) | O(n) | O(1)* | O(1)* | *위치 노드를 이미 알 때. 탐색까지 포함하면 O(n) |
| 스택/큐(Stack/Queue) | O(n) | O(n) | O(1) | O(1) | push/pop, enqueue/dequeue는 O(1) |
| 해시 테이블(Hash Table) | - | O(1) / O(n) | O(1) / O(n) | O(1) / O(n) | 평균 O(1), 충돌이 심하면 최악 O(n) |
| 이진 검색 트리(BST) | - | O(log n) / O(n) | O(log n) / O(n) | O(log n) / O(n) | 편향되면 최악 O(n) |
| 균형 트리(AVL·RB·B-Tree) | - | O(log n) | O(log n) | O(log n) | 균형을 유지해 최악도 O(log n) |
| 힙(Heap) | - | O(n) | O(log n) | O(log n) | 최소/최대값 조회는 O(1) |

> 표기는 '평균 / 최악'(단일 값은 평균=최악). 빈칸(-)은 해당 연산이 그 자료구조의 주 용도가 아님을 뜻한다.
...
- 해시(Hash) 함수
  - SHA-256, SHA-1, MD5 등이 있다. (단 SHA-1·MD5는 충돌 취약점으로 보안 용도에서는 폐기되었고, 무결성·보안에는 SHA-256 이상을 쓴다.) 자료구조라기보다 해시 테이블·체크섬 등에 쓰이는 함수다.
