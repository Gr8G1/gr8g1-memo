...
3. 관계(Relationship)
   ...
   - 관계의 종류
     - 종속(Dependent) 관계
       - 두 개체 사이의 주-종 관계를 표현, 식별과 비식별 관계가 있다.
     - 중복(Redundant) 관계
       - 두 개체 사이의 2번 이상의 종속 관계가 존재하는 관계이다.
     - 재귀(Recursive) 관계
       - 자기 자신과 관계를 갖는것으로 순환 관계라고도 한다.
     - 배타(Exclusive) 관계
       - 개체의 속성이나 구분자를 기준으로 개체의 특성을 분할하는 관계로 배타 AND, OR 관계로 구분한다.
       - 배타 AND
...
함수적 종속 등의 종속성 이론을 활용하여 설계된 스키마를 더 나은 스키마로 개선하는 과정이다.
...
- 테이블 분할
  - 테이블을 수직(속성이 너무 많을시) 또는 수평(레코드별 사용 빈도수가 상이할 시) 분할하여 성능 향상에 도움이 될 경우 수행
- 중복 테이블 추가
...
무결성(Integrity) 보장을 위해 트랜잭션이 가져야할 특성 4가지
...
#### 뷰(View)
하나 이상의 테이블에서 유도된 **가상 테이블(virtual table)** 이다. 실제 데이터를 따로 저장하지 않고, 정의된 SELECT 쿼리의 결과를 테이블처럼 보여준다.
- 복잡한 조인·집계 쿼리를 미리 정의해 두고 간단히 재사용할 수 있으며, 특정 컬럼만 노출해 **보안(접근 제어)** 에 활용한다.
- 원본 테이블이 바뀌면 뷰 결과도 자동 반영된다. (단순 뷰는 갱신도 가능하나 조인·집계가 들어간 뷰는 보통 읽기 전용)

#### 인덱스(Index)
테이블의 특정 컬럼 값을 빠르게 찾기 위한 **자료구조(주로 B-Tree)** 로, 책의 '찾아보기'에 해당한다. 풀 스캔 대신 인덱스를 타면 조회가 크게 빨라진다.
- **장점**: SELECT(특히 WHERE·JOIN·ORDER BY) 성능 향상.
- **대가**: 인덱스도 저장 공간을 차지하고, INSERT/UPDATE/DELETE 시 인덱스도 함께 갱신해야 하므로 **쓰기 성능이 떨어진다.** 따라서 조회가 잦고 카디널리티(고유값 다양성)가 높은 컬럼에 선별적으로 건다.
- **클러스터드(Clustered) 인덱스**: 인덱스 순서대로 데이터(행)가 물리 정렬됨(테이블당 1개, 보통 PK). **논클러스터드(Non-clustered) 인덱스**: 데이터와 별도로 키→위치를 가리키는 인덱스(여러 개 가능).

#### 저장 프로시저(Stored Procedure)
일련의 SQL를 마치 하나의 함수처럼 실행하기 위한 쿼리의 집합체이다. 미리 컴파일되어 DB에 저장되며 `CALL`로 호출한다. 값을 직접 반환하지 않고 OUT 파라미터로 결과를 넘기며, 주로 여러 INSERT/UPDATE 같은 일련의 처리를 캡슐화한다.

#### 저장 함수(Stored Function)
저장 프로시저와 유사하지만 **반드시 하나의 값을 반환(RETURN)** 하며, SQL 문 안에서 일반 함수처럼 호출할 수 있다(예: `SELECT my_func(col) FROM ...`). 계산·변환 로직을 함수로 묶어 쿼리에서 재사용할 때 쓴다.
